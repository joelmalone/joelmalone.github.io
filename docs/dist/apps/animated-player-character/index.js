...
import { PointerEventTypes } from '../../../_snowpack/pkg/@babylonjs/core/Legacy/legacy.js';
import '../../../_snowpack/pkg/@babylonjs/loaders/glTF.js';
import CharacterGLB from './apc.glb.proxy.js';
const SPEED = 1.6;
/**
...
function createInputMap(scene) {
  ...
  const observables = {
    forward: new BABYLON.Observable(),
    left: new BABYLON.Observable(),
    right: new BABYLON.Observable(),
    all: new BABYLON.Observable()
  };
  const mapping = {
    w: 'forward',
    a: 'left',
...
export function createScene(engine, canvasElement) {
  ...
  var light2 = new BABYLON.DirectionalLight('dir01', new BABYLON.Vector3(3, -3, 1), scene);
  light2.position = new BABYLON.Vector3(0, 5, 5);
  var shadowGenerator = new BABYLON.ShadowGenerator(1024, light2);
  shadowGenerator.useBlurExponentialShadowMap = true;
  shadowGenerator.blurKernel = 32;
  const followCamera = new BABYLON.FollowCamera('camera1', new BABYLON.Vector3(), scene);
  followCamera.radius = 5;
  followCamera.heightOffset = 1.5; // followCamera.attachControl(true);
  ...
  helper.ground.position.y += 0.01; // Import the GLB

  BABYLON.SceneLoader.ImportMesh('', CharacterGLB, '', scene, (meshes, particleSystems, skeletons, animationGroups, transformNodes, geometries, lights) => {
    shadowGenerator.addShadowCaster(meshes[0], true); // Get the animations and begin playing them, but with 0 weight

    const idleAnimation = animationGroups.find(a => a.name === 'Idle');
    const walkingAnimation = animationGroups.find(a => a.name === 'Walking');
    const leftAnimation = animationGroups.find(a => a.name === 'Left');
    const rightAnimation = animationGroups.find(a => a.name === 'Right');
    [idleAnimation, walkingAnimation, leftAnimation, rightAnimation].forEach(a => {
    ...
    const weightsKeys = ['idleAnimation', 'walkingAnimation', 'leftAnimation', 'rightAnimation'];
    const animationSubscription = scene.onBeforeAnimationsObservable.add(() => {
      ...
      targetWeights.idleAnimation = targetWeights.walkingAnimation == 0.000001 && targetWeights.leftAnimation == 0 && targetWeights.rightAnimation == 0 ? 1 : 0; // Transition to the target weights
      ...
      walkingAnimation.setWeightForAllAnimatables(currentWeights['walkingAnimation']);
      leftAnimation.setWeightForAllAnimatables(currentWeights['leftAnimation']);
      rightAnimation.setWeightForAllAnimatables(currentWeights['rightAnimation']);
    }); // Move the player

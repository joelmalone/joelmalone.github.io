// from https://github.com/jongomez/ragdoll.js
export function Ragdoll(skeleton, mesh, config, jointCollisions = false, showBoxes = false, mainPivotSphereSize = 0, disableBoxBoneSync = false) {
        ...
        for (let i = 0; i < this.config.length; i++) {
            ...
            for (let ii = 0; ii < boneNames.length; ii++) {
                ...
                if(currentBone == undefined) {
                    console.error("Bone", boneNames[ii], "does not exist :(");
                    return;
                }
                ...
                if(this.config[i].hasOwnProperty("putBoxInBoneCenter") && this.config[i].putBoxInBoneCenter || this.putBoxesInBoneCenter) {
                    ...
                    if(currentBone.length === undefined) {
                        console.warn("The length property is not defined for bone", currentBone.name, ". putBox(es)InBoneCenter will not work :(");
                    }
    ...
    this.initJoints = function() {
        ...
        for (let i = 0; i < this.bones.length; i++) {
            ...
            if (nearestParent == null) {
                console.error("Couldn't find a nearest parent bone in the configs for bone called", this.boneNames[i], ":(((");
                return;
            }
    ...
    this.addImpostorRotationToBone = function(boneIndex) {
        ...
        // this.bones[boneIndex].setRotationQuaternion(BABYLON.Quaternion.Identity());
    }
    ...
    this.defineRootBone = function() {
        ...
        if (skeletonRoots.length != 1) {
            console.error("Ragdoll creation failed: there can only be 1 root in the skeleton :(");
            return false;
        }

        this.rootBoneName = 'mixamorig:Hips';//skeletonRoots[0].name;
        this.rootBoneIndex = this.boneNames.indexOf(this.rootBoneName);
        if (this.rootBoneIndex == -1) {
            console.error("Ragdoll creation failed: the array boneNames doesn't have the root bone in it :( - the root bone is", this.skeleton.getChildren());
            return false;
        }
    ...
    this.init = function() {
        ...
        this.syncBonesAndBoxes = () => {
            ...
            if (this.ragdollMode) {
                ...
                // }
                for (let i = 0; i < this.bones.length; i++) {
                    this.impostors[i].syncBoneWithImpostor(this.bones[i], this.mesh, null)
                }
                // this.impostors[0].syncBoneWithImpostor(this.bones[0], this.mesh, null)
            } else {
